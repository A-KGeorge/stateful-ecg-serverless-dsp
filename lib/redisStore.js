 ...
 * @returns {number} Number of pending messages in the stream
 */
export async function getQueueDepth() {
  try {
    const info = await redis.xpending(ECG_STREAM, CONSUMER_GROUP);
    // info returns [count, firstId, lastId, consumers]
    return info[0] || 0;
  } catch (err) {
    // If error, assume no pending messages
    return 0;
  }
}
